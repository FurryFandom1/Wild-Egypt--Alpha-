    ...
    private void Awake()
{
    ...
    {
        // Переносим инициализацию камеры только для локального игрока
    ...
    void Start()
{
    ...
    if (photonView.IsMine)
    {
        ...
        // Инициализация sprintBar
        sprintBarCG = GetComponentInChildren<CanvasGroup>();
    }
    else
    {
        // Отключаем ненужные компоненты для удалённых игроков
        if (playerCamera != null) 
        {
        ...
        this.enabled = false; // Отключаем весь контроллер
    }
}
    ...
    private void Update()
    {
        ...
        if (playerCamera != null && cameraCanMove)
        {
